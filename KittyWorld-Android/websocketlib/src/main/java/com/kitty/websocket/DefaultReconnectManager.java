...
public class DefaultReconnectManager implements ReconnectManager {
    ...
    public void startReconnect() {
        if (reconnecting) {
            LogUtil.i("WSWrapper", "Reconnecting, do not call again.");
            return;
        }
        if (destroyed) {
            LogUtil.e("WSWrapper", "ReconnectManager is destroyed!!!");
            return;
        }
        ...
        try {
            ...
            LogUtil.e("WSWrapper", "线程队列已满，无法执行此次任务。", e);
            reconnecting = false;
        }
    }
    ...
    private Runnable getReconnectRunnable() {
        return new Runnable() {
            ...
            public void run() {
                ...
                LogUtil.d("WSWrapper", "开始重连:" + reconnectCount);
                reconnectCount++;
                reconnecting = true;
                connected = false;
                try {
                    ...
                    //重连失败
                    LogUtil.i("WSWrapper", "reconnectOnce failed!");
                    mOnDisconnectListener.onDisconnect();
                } finally {
                    LogUtil.d("WSWrapper", "重连结束:" + finishCount);
                    finishCount++;
                    reconnecting = false;
                    LogUtil.i("WSWrapper", "reconnecting = false");
                }
            }
        };
    }
    ...
    public void onConnected() {
        connected = true;
        synchronized (BLOCK) {
            LogUtil.i("WSWrapper", "onConnected()->BLOCK.notifyAll()");
            BLOCK.notifyAll();
        }
    }
    ...
    public void onConnectError(Throwable th) {
        connected = false;
        synchronized (BLOCK) {
            LogUtil.i("WSWrapper", "onConnectError(Throwable)->BLOCK.notifyAll()");
            BLOCK.notifyAll();
        }
    }
