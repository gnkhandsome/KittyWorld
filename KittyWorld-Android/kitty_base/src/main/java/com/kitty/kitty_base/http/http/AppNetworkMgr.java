...
import com.kitty.kitty_base.http.KLog;
        ...
        if (context != null) {
            ...
            if (mNetworkInfo != null) {
                ...
                if (ConnectivityManager.TYPE_MOBILE == mNetworkInfo.getType()) {
                    KLog.i("AppNetworkMgr", "网络连接类型为：TYPE_MOBILE");
                    //判断移动网络连接状态
                    ...
                    if (STATE_MOBILE == NetworkInfo.State.CONNECTED) {
                        KLog.i("AppNetworkMgrd", "网络连接类型为：TYPE_MOBILE, 网络连接状态CONNECTED成功！");
                        return mNetworkInfo.isAvailable();
                    }
                }
                //判断是TYPE_WIFI网络
                if (ConnectivityManager.TYPE_WIFI == mNetworkInfo.getType()) {
                    KLog.i("AppNetworkMgr", "网络连接类型为：TYPE_WIFI");
                    //判断WIFI网络状态
                    ...
                    if (STATE_WIFI == NetworkInfo.State.CONNECTED) {
                        KLog.i("AppNetworkMgr", "网络连接类型为：TYPE_WIFI, 网络连接状态CONNECTED成功！");
                        return mNetworkInfo.isAvailable();
                    }
                }
            }
